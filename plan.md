...
## Repository & Data Encryption — Architecture Plan

### Problem Constraints

Application-level Git encryption (git-crypt, git-remote-gcrypt) would break:
- Web-based `.als` diffing (decrypted XML comparison)
- Metadata/README rendering on the web app
- Gitea's native file browsing and LFS serving

Encryption must be **transparent** — data is encrypted at rest but available in cleartext to the running services.

### Layer 1 — Volume / Disk Encryption (Repository Storage)

| Concern | Recommendation |
|--|--|
| **What to encrypt** | The Docker volume mounted at `/opt/soundhaus/gitea/git/repositories/` (host path) → `/data/git/repositories/` (container) |
| **Mechanism** | **LUKS full-disk encryption** on the Digital Ocean block storage volume attached to the droplet. DO does not encrypt volumes by default. |
| **Implementation** | 1. Provision a new DO Block Storage volume. 2. Format with `cryptsetup luksFormat /dev/disk/by-id/<vol>`. 3. Open with `cryptsetup open ... gitea_crypt`. 4. Create ext4 on `/dev/mapper/gitea_crypt`, mount at `/opt/soundhaus/gitea/git/repositories/`. 5. Add to `/etc/crypttab` with a keyfile stored in `/root/` (mode 0400). 6. Migrate existing repo data with rsync. |
| **Key management** | Store the LUKS passphrase in **1Password** or equivalent secrets vault. The keyfile on disk enables unattended boot; the passphrase is the disaster-recovery backup. |
| **Alternative (if migrating to AWS)** | Use **EBS encryption** (AES-256, AWS KMS-managed). Enabled per-volume at creation time — zero application changes. |
| **LFS object store** | If using Digital Ocean Spaces (S3-compatible), enable **server-side encryption (SSE-S3)**. If migrating to Cloudflare R2, objects are encrypted at rest by default. |

### Layer 2 — Column-Level Database Encryption (Supabase / PostgreSQL)

| Concern | Recommendation |
|--|--|
| **What to encrypt** | Columns containing: OAuth/access tokens, Gitea admin token references, PAT hashes, invitation tokens, webhook secrets. |
| **Mechanism** | Use **pgcrypto** `pgp_sym_encrypt` / `pgp_sym_decrypt` with an application-level symmetric key. |
| **Key storage** | `ENCRYPTION_KEY` env var in `.env.remote`, loaded via `config.py`. Never committed. |
| **Which tables / columns** | `webhook_configs.webhook_secret`, `collaborator_invitations.invitation_token`, any future OAuth refresh tokens. |
| **Migration path** | 1. `ALTER TABLE ... ADD COLUMN <col>_encrypted BYTEA;` 2. Backfill: `UPDATE ... SET <col>_encrypted = pgp_sym_encrypt(<col>, $KEY);` 3. Drop plaintext column after verification. 4. Update `services/` to decrypt on read. |
| **Read path in FastAPI** | Use a thin helper: `db.execute(text("SELECT pgp_sym_decrypt(col, :key)"), {"key": settings.encryption_key})` — keeps encryption transparent to Pydantic models. |
| **Alternative** | **Supabase Vault** (available on Pro plan) — stores secrets in `vault.secrets` with automatic column-level encryption managed by Supabase. Zero FastAPI changes for vault-stored secrets. |

### Layer 3 — Transport Encryption (Already in Place)

- HTTPS (TLS 1.2+) enforced on `thesound.haus` via Caddy/Nginx reverse proxy.
- Git over SSH (port 2222) for desktop push/pull.
- Internal Docker network traffic (FastAPI ↔ Gitea) is not TLS but is isolated to the host bridge — acceptable at current scale.

### What NOT To Do

- **Do not** use `git-crypt` or GPG-based per-file encryption — breaks web diffing and Gitea file browsing.
- **Do not** encrypt the Supabase `profiles` table wholesale — it must remain queryable for username search, leaderboards, etc.
- **Do not** roll custom encryption for passwords — Supabase Auth already uses bcrypt internally.
