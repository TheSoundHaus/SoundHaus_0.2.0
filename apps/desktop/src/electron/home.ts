...
async function getGiteaApiRequestOptions(): Promise<{ protocol: string; hostname: string; port: number; basePath: string }> {
    const remote = await getAllowedCloneRemote();
    if (!remote) throw new Error('No Gitea URL configured. Please log in first.');
...
async function init(folderPath: string, projectInfo?: ProjectSetupData): Promise<string> {
    ...
    try {
        ...
        const giteaRequestTarget = await getGiteaApiRequestOptions();
